### LMER for Sham vs. Full TBI in a 1-choice intercept-only linear model subsetted to P2 only
...
#import TBI data (choose 1 sample/effect size)
...
data<-subset(data, ChoiceOption==2)
...
get_LMER_aov=function(rep){
  ...
  m1<-lmer(scale(asin(sqrt(PctChoice/100)))~1+(1|Subject), data=rep_data)
  m2<-lmer(scale(asin(sqrt(PctChoice/100)))~Injury+(1|Subject), data=rep_data)
