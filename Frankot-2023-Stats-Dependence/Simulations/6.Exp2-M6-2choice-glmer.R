### LMER for Sham vs. Full TBI in a 2-choice intercept-only glmer model
...
#import TBI data (one effect size/sample size at a time)
...
data$P2 <- ifelse(data$ChoiceOption == 2, 1, 0)
temp <- aggregate(ChoiceCount~P2+Subject+Session+Repetition, FUN=sum, data=data)
colnames(temp)[colnames(temp)=="ChoiceCount"] <- "binom_count"
data<-merge(data, temp, by=c("Subject", "Session", "Repetition", "P2"))
...
get_LMER_aov=function(rep){
  ...
  m1 <- glmer(binom_count/TotChoice ~ 1+(1|Subject), 
              data=subset(rep_data,P2==1), 
              weights=TotChoice, family=binomial,
              control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
  m2 <- glmer(binom_count/TotChoice ~ Injury+(1|Subject), 
                      data=subset(rep_data,P2==1), 
                      weights=TotChoice, family=binomial,
                      control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
