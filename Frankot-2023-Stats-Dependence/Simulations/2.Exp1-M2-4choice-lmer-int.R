### LMER for Sham vs. Full TBI in a 4-choice intercept-only linear model
...
#import TBI data (choose 1 sample size/effect size)
...
#function to perform model comparison between LMERS with and without the TBI*Choice Interaction
get_LMER_aov=function(rep){
  rep_data=subset(data, Repetition==rep)
  m1<-lmer(scale(asin(sqrt(PctChoice/100)))~ChoiceOption+Injury+
             (0+dummy(ChoiceOption, "2")|Subject), 
           data=rep_data)
  m2<-lmer(scale(asin(sqrt(PctChoice/100)))~ChoiceOption*Injury+
             (0+dummy(ChoiceOption, "2")|Subject), 
           data=rep_data)
